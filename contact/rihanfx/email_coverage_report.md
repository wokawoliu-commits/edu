...
本轮按 Gravity 新增策略重新扫描所有仍缺邮箱的公司：在静态抓取和 Playwright 渲染复核基础上，额外提高 `Global Network`、`Overseas Offices`、`Subsidiaries`、`Branches`、`globalnetwork` 等路径和链接文本的优先级。
...
本轮对缺邮箱公司增加了浏览器渲染复核，并额外强化全球网络/海外分支页面检查。多数 A/B 类大厂在渲染后仍没有公开可用邮箱，说明它们更依赖官网表单、IR、LinkedIn、投资/发行入口和暖介绍。
...
新版策略已经覆盖“静态抓不到但页面渲染后可见”和“邮箱藏在 Global Network / 海外分支页”的情况；本轮对剩余 53 家缺口复跑后没有新增安全邮箱。当前公开邮箱覆盖率停在 33.8%。

如果要继续提高可触达率，下一步应从“找邮箱”转向“找路径”：
